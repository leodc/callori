...
6. Run the controlled real-call acceptance cases in [VERIFICATION](VERIFICATION.md) before considering changed voice behavior accepted.
...
```sh
npm run backup -- /private/backup-location/callori-2026-09-15.sqlite
```
...
5. Start Callori and inspect Settings/history. Restored unfinished calls are marked interrupted; they are never redialed or represented as resumed sessions. Persisted carrier calls are reconciled by attempting to end them.
...
- **Shutdown deadline:** Callori attempts cleanup and exits within 20 seconds. Recovery records remain on disk if cleanup does not complete. The Telnyx time cap is the final fallback, not an assertion that immediate hangup succeeded.
...
The SQLite database and profile persist indefinitely until managed by the operator. User-facing retention/deletion and history pagination are TODOs. There is no application-level encryption at rest and no audio recording by Callori. Verify provider-side recording/retention separately.
