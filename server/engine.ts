...
import { z } from "zod";
import type { Call, CallInput, Transcript } from "../lib/types";
...
function later(id: string, ms: number, fn: () => void) {
  ...
  const timer = setTimeout(() => {
    ...
    if (!terminal(getCall(id)!.status)) fn();
  }, ms);
  s.timers.add(timer);
}
...
export async function createCall(
  ...
  if (
    pendingControls().some((row) => {
      const c = getCall(row.id);
      return !c || terminal(c.status);
    })
  )
    throw new Error("RECOVERY_PENDING");
  if (calls().some((c) => !terminal(c.status))) throw new Error("ACTIVE_CALL");
  if (input.mode === "live") {
  ...
  saveCall(call);
  // Persist before dialing: a lost HTTP response must never cause a second phone call.
  if (requestKey) saveRequest(requestKey, call.id);
  const s: Session = {
  ...
  later(
    call.id,
    Math.min(1200, Math.max(30, Number(process.env.MAX_CALL_SECONDS) || 600)) *
      1000,
  ...
  try {
    ...
    const data = await telnyx("/calls", {
      ...
      command_id: call.id,
      timeout_secs: 45,
      time_limit_secs: Math.min(
        1200,
        Math.max(30, Number(process.env.MAX_CALL_SECONDS) || 600),
      ),
    ...
    if (!s.remoteEnded) {
      ...
      saveControl(call.id, s.controlId!);
    }
    later(call.id, 55000, () => {
...
export function answerUser(id: string, qid: string, answer: string) {
  const call = getCall(id),
    s = sessions.get(id);
...
export async function endCall(
  ...
  const task = performEndCall(id, status, error);
  if (s) s.endTask = task;
  try {
    await task;
  } finally {
...
async function performEndCall(
  ...
  if (call.mode === "live" && s?.controlId) {
    try {
      await hangup(s.controlId, (s.hangupCommand ??= hangupCommand(id)));
    } catch {
      if (!s?.remoteEnded) {
  ...
  socket.on("message", (raw) => {
    ...
    try {
      ...
      if (event.event === "start" && !started) {
        ...
        playbackComplete = true;
      else if (event.event === "stop") void remoteHangup(id);
...
export async function recoverOrphanedCalls() {
  ...
  recovering = true;
  try {
    for (const row of pendingControls()) {
      const call = getCall(row.id);
      if (call && !terminal(call.status)) continue;
      try {
        await hangup(row.control_id, hangupCommand(row.id));
        clearControl(row.id);
      } catch {
        console.warn(
