...
const dir = mkdtempSync(join(tmpdir(), "callori-engine-"));
...
test("persistent state, concurrency guard, question correlation and cancellation", async () => {
  const c = await engine.createCall(input);
  assert.equal(c.status, "dialing");
  assert.equal(store.getCall(c.id)?.objective, input.objective);
  await assert.rejects(() => engine.createCall(input), /ACTIVE_CALL/);
  engine.askUser(c.id, "Are you taking any medication?", "information");
  const q = store.getCall(c.id)!.question!;
  assert.equal(store.getCall(c.id)?.status, "waiting");
  assert.throws(
    () => engine.answerUser(c.id, "wrong-question", "No"),
    /STALE_QUESTION/,
  );
  engine.sessions.get(c.id)!.ai = new FakeSocket() as unknown as WebSocket;
  engine.answerUser(c.id, q.id, "I am not taking medication.");
  assert.equal(store.getCall(c.id)?.status, "connected");
  assert.equal(
    store.getCall(c.id)?.question?.answered,
  ...
  assert.equal(store.getCall(c.id)?.status, "cancelled");
  assert.equal(engine.sessions.has(c.id), false);
  assert.throws(() => engine.answerUser(c.id, q.id, "Late"), /CALL_ENDED/);
});
test("live bridge relays audio, asks the user, resumes the same session and completes", async () => {
  ...
  const c = await engine.createCall({ ...input, mode: "live", language: "en" });
  ...
  await settle();
  assert.equal(
    store.getCall(c.id)?.transcript[0].translations.es,
    "Texto traducido",
  );
  ai.event({
  ...
  await settle();
  await settle();
  const agentLine = store
    .getCall(c.id)!
    .transcript.find((x) => x.id === "audio-1")!;
  assert.equal(
  ...
  engine.answerUser(c.id, q.id, "No. Pregunta por el jueves.");
  assert.equal(
    ai.sent.length,
    before + 1,
  ...
  await settle();
  assert.equal(store.getCall(c.id)?.result?.outcome, "incomplete");
  await engine.endCall(c.id, "completed");
  assert.equal(store.getCall(c.id)?.status, "completed");
  assert.equal(engine.sessions.has(c.id), false);
...
test("restart recovery terminates persisted provider calls instead of silently resuming", async () => {
  ...
  assert.equal(store.getCall(c.id)?.status, "failed");
  assert.equal(store.getCall(c.id)?.error, "SERVER_RESTART");
  assert.equal(store.getControl(c.id), undefined);
});
test("webhook and HTTP idempotency records survive duplicate delivery", () => {
  assert.equal(store.seenEvent("event-1"), false);
  assert.equal(store.seenEvent("event-1"), true);
  const c = store.calls()[0];
  store.saveRequest("request-1", c.id);
  assert.equal(store.requestCall("request-1")?.id, c.id);
});
...
test("cancellation waits for a pending dial and hangs up the late carrier response", async () => {
  ...
  globalThis.fetch = async (url, init) =>
    String(url) === "https://api.telnyx.com/v2/calls"
      ...
      : mockFetch(url, init);
  try {
    const creating = engine.createCall(input);
...
test("failed recovery stays durable, blocks redial, and reuses the hangup command across retries", async () => {
  ...
  try {
    ...
    await engine.recoverCalls();
    assert.equal(store.getCall(call.id)?.status, "failed");
    assert.ok(store.getControl(call.id));
    const command = store.hangupCommand(call.id);
    assert.notEqual(command, call.id);
    await assert.rejects(() => engine.createCall(input), /RECOVERY_PENDING/);
    globalThis.fetch = mockFetch;
...
test("cancelled model responses cannot execute tools and terminal calls ignore queued tool work", async () => {
  ...
  await settle();
  assert.equal(store.getCall(call.id)?.question, undefined);
  ai.event({
    type: "response.done",
    response: {
      status: "completed",
